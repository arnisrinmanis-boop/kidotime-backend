...
def pc_heartbeat(token: str, key=Depends(verify_key)):
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT id, registered FROM pcs WHERE token=%s", (token,))
    row = c.fetchone()
    if not row:
...
@app.get("/api/parent-pin")
def get_parent_pin(key=Depends(verify_key)):
    conn = get_db(); c = conn.cursor()
    c.execute("SELECT value FROM settings WHERE key='parent_pin'")
    row = c.fetchone(); conn.close()
    return {"pin": row[0] if row else None}

@app.post("/api/parent-pin")
def set_parent_pin(body: dict, key=Depends(verify_key)):
    pin = body.get("pin")
    if not pin or not str(pin).isdigit() or len(str(pin)) != 4:
        raise HTTPException(400, "PIN must be exactly 4 digits")
    conn = get_db(); c = conn.cursor()
    c.execute("INSERT INTO settings (key, value) VALUES ('parent_pin', %s) ON CONFLICT (key) DO UPDATE SET value=%s",
              (pin, pin))
    conn.commit(); conn.close(); return {"ok": True}
